...
#Extract and prepare STS files for sharing
...
var_list='pr tsmax tsmin prmax ps sund psmin psavg mrros mrro tasmax tasmin tas sfcWindmax' #Variables in STS

#--------------------------------------------------------
#Generate list of experiments ran
cat << EOF > sts.explist.csv
...
allfile_list=`ls ${datadir}/*STS.??????????.nc`
...
linenum=`cat sts.explist.csv | cut -d "," -f 2- | grep -n "${cup},${pbl},${moist}" | cut -d ":" -f 1`
[[ ${linenum} != '' ]] && expnum=`sed -n ${linenum}p < sts.explist.csv | cut -d "," -f 1` || expnum='NA'
...
for var in ${var_list}; do
        ...
        for ifile in ${allfile_list}; do
                ...
                cdo selvar,${var} ${ifile} ${odir}/${var}_exp${expnum}.${ofiledate}.ststmp1
        done

        for (( y=${year_start}; y<=${year_end}; y++ )); do

                ifile_list=`ls ${odir}/${var}_exp${expnum}.${y}??????.ststmp1`
                cdo -O mergetime ${ifile_list} ${odir}/${var}_exp${expnum}.${y}.ststmp2
                        rm -rf ${ifile_list}
        done

        cdo -O mergetime ${odir}/${var}_exp${expnum}.????.ststmp2 ${odir}/${var}_exp${expnum}.ststmp3
                rm -rf ${odir}/${var}_exp${expnum}.????.ststmp2

        cdo setgridtype,lonlat -selindexbox,${indexbox} ${odir}/${var}_exp${expnum}.ststmp3 ${odir}/${var}_exp${expnum}_daily.nc
                rm -rf ${odir}/${var}_exp${expnum}.ststmp3
