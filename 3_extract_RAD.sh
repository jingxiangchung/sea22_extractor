...
#Extract and prepare RAD files for sharing
...
var_list='rlut prw'  #Variables in 3-hourly

#--------------------------------------------------------
#Generate list of experiments ran
cat << EOF > rad.explist.csv
...
allfile_list=`ls ${datadir}/*RAD.??????????.nc`
...
linenum=`cat rad.explist.csv | cut -d "," -f 2- | grep -n "${cup},${pbl},${moist}" | cut -d ":" -f 1`
[[ ${linenum} != '' ]] && expnum=`sed -n ${linenum}p < rad.explist.csv | cut -d "," -f 1` || expnum='NA'
...
for var in ${var_list}; do
        ...
        for ifile in ${allfile_list}; do
                ...
                cdo selvar,${var} ${ifile} ${odir}/${var}_exp${expnum}.${ofiledate}.radtmp1
        done

        for (( y=${year_start}; y<=${year_end}; y++ )); do

                ifile_list=`ls ${odir}/${var}_exp${expnum}.${y}??????.radtmp1`
                cdo -O mergetime ${ifile_list} ${odir}/${var}_exp${expnum}.${y}.radtmp2
                        rm -rf ${ifile_list}
        done

        cdo -O mergetime ${odir}/${var}_exp${expnum}.????.radtmp2 ${odir}/${var}_exp${expnum}.radtmp3
                rm -rf ${odir}/${var}_exp${expnum}.????.radtmp2

        cdo setgridtype,lonlat -selindexbox,${indexbox} -monmean ${odir}/${var}_exp${expnum}.radtmp3 ${odir}/${var}_exp${expnum}_monmean.nc
                rm -rf ${odir}/${var}_exp${expnum}.radtmp3
